...
public class Connection {
    ...
    public <ErrorResponse> InputStream getThumbInputStream(String thumb, INotifiableManager manager) throws FileNotFoundException, MalformedURLException {
        ...
        } catch (IOException | NoSettingsException e) {
            ErrorResponse errorResponse = (ErrorResponse) convertToErrorResponse(e); // Convert the caught exception to ErrorResponse
            manager.onError((Client.ErrorResponse) errorResponse); // Handle the error using the manager
        }
        return null;
    }

    private Client.ErrorResponse convertToErrorResponse(Exception e) {
        String errorMessage = e.getMessage(); // Get the error message from the exception
        int errorCode;

        // Determine the error code based on the type of exception
        if (e instanceof FileNotFoundException) {
            errorCode = 404; // File not found error
        } else if (e instanceof MalformedURLException) {
            errorCode = 400; // Malformed URL error
        } else if (e instanceof NoSettingsException) {
            errorCode = 500; // No settings error
        } else {
            errorCode = 500; // Default error code for other exceptions
        }

        // Create the ErrorResponse object with the error code and message
        return new Client.ErrorResponse(errorCode, errorMessage);
    }
    ...
    public JsonNode query(String command, JsonNode parameters, INotifiableManager manager) {
        ...
        try {
            ...
            try (JsonGenerator jg = jsonFactory.createJsonGenerator(uc.getOutputStream(), JsonEncoding.UTF8)) {
                jg.setCodec(mapper);
            ...
            try (JsonParser jp = jsonFactory.createJsonParser(uc.getInputStream())) {
                jp.setCodec(mapper);
                return jp.readValueAs(JsonNode.class);
            }

        } catch (MalformedURLException | NoSettingsException e) {
            manager.onError(e); // Pass the exception to the manager
        } catch (IOException e) {
            handleIOException(e, uc, manager); // Handle IOException separately
        }
        return new ObjectNode(null);
    }

    private void handleIOException(IOException e, URLConnection uc, INotifiableManager manager) {
        int responseCode = -1;
        try {
        ...
        if (responseCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
            manager.onError(new IOException("Unauthorized access"));
        } else {
            manager.onError(e);
        }
    }
