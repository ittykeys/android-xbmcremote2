...
public class TvShowManager extends AbstractManager implements ITvShowManager,
    ...
    public ArrayList<TvShow> getTvShows(Context context) {
        try {
            ...
            // Convert WifiStateException to an Exception and pass it to onError
            Exception exception = createErrorResponse(e);
            TvShowManager.this.onError(exception);
        }
        return new ArrayList<TvShow>();
    }


    private Exception createErrorResponse(WifiStateException e) {
        return new Exception("Error code: " + ERROR_CODE + ", Message: " + e.getMessage());
    }
    ...
    public ArrayList<Season> getAllSeasons(Context context) {
        try {
            ...
            // Convert WifiStateException to a Response object and pass it to onError
            Exception response = createErrorResponse(e);
            TvShowManager.this.onError(response);
        }
