...
public abstract class AbstractManager implements INotifiableManager {
    ...
    private Exception extractExceptionFromResponse(Response<?> response) {
        // Extract relevant information from the Response object
        int statusCode = response.code();
        String errorMessage = response.message();

        // Create an appropriate Exception based on the information
        return new Exception("Error with status code " + statusCode + ": " + errorMessage);
    }


    public void onError(Exception response) {
        if (mController != null) {
            // Directly pass the exception to the controller
            mController.onError(response);
        }
    }
